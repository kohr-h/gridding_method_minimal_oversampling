...
class projectors:
    ...
    def __init__(
        ...
        W, lut, deapod = glut.configure_regridding(
            npix, kernel, oversampl, interp, W, errs
        )
        ...
        param_nofilt = np.array(
            [ctr, 0, 0, oversampl, interp1, len(lut) - 5, W, radon_degree]
        )

        ##  Setting for filtered backprojection
        param_filt = np.array(
            [ctr, filt, 0, oversampl, interp1, len(lut) - 5, W, radon_degree]
        )
    ...
    ##  Forward projector
    def A(self, x):
        return grid.forwproj(
            x.astype(myfloat), self.angles, self.param_nofilt, self.lut, self.deapod
        )

    ##  Backprojector
    def At(self, x):
        return grid.backproj(
            x.astype(myfloat), self.angles, self.param_nofilt, self.lut, self.deapod
        )

    ##  Filtered backprojection (with ramp filter as default)
    def fbp(self, x):
        return grid.backproj(x, self.angles, self.param_filt, self.lut, self.deapod)
