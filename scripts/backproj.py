...
def getArgs():
    parser = argparse.ArgumentParser(
        description="Tomographic gridding reconstruction",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument(
        "-Di", dest="pathin", default="./", help="Specify path to input data"
    )
    ...
    parser.add_argument(
        "-o", dest="fileout", help="Specify name of output reconstruction"
    )
    ...
    parser.add_argument(
        "-dpc", dest="dpc", action="store_true", help="Enable DPC reconstruction"
    )

    parser.add_argument(
        "-dbp", dest="dbp", action="store_true", help="Enable DBP reconstruction"
    )
    ...
    parser.add_argument(
        "-Z", dest="edge_pad", action="store_true", help="Enable edge padding"
    )
...
def main():
    ...
    if args.filein is None:
        ...
        for i in range(1, nimg):
            pool.apply_async(
                multi_thread, (pathin, pathout, file_list[0][i], args, [i1, i2])
            )
        pool.close()
        pool.join()
    time2 = time.time()
