...
def create_pseudo_polar_angles(nang):
    ...
    pseudo_angles[int(n / 2) : int(n / 4) : -1] = (
        np.pi / 2 - pseudo_angles[0 : int(n / 4)]
    )
...
def create_projection_angles(
    nang=None, pseudo=0, start=0.0, end=180.0, wedge=False, textfile=None
):
