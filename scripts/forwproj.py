...
def getArgs():
    parser = argparse.ArgumentParser(
        description="Tomographic gridding forward projection",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument(
        "-Di", dest="pathin", default="./", help="Specify path to input image"
    )

    parser.add_argument("-i", dest="filein", help="Specify name of input sinogram")

    parser.add_argument("-Do", dest="pathout", help="Specify path to output data")

    parser.add_argument("-o", dest="fileout", help="Specify name of output sinogram")

    parser.add_argument(
        "-n", dest="nang", type=np.int, help="Specify number of projection angles"
    )
    ...
    parser.add_argument(
        "-dpc", dest="dpc", action="store_true", help="Enable DPC projector"
    )
...
def main():
    ...
    if args.geometry == "0" or args.geometry == "1":
        ...
        angles = utils.create_projection_angles(
            nang, angle_type, angle_start, angle_end
        )
    ...
    if args.filein is None:
        ...
        for i in range(1, nimg):
            pool.apply_async(
                multi_thread, (pathin, pathout, file_list[0][i], angles, args)
            )
        pool.close()
        pool.join()
    time2 = time.time()
