...
def sino_correct_rot_axis(sino, ctr):
    ...
    if shift.is_integer() is False:
        ...
        sino_corr = interp.griddata(
            z, sino_corr.reshape(-1), (grid_x2, grid_y2), method="cubic"
        )
...
def select_resol_square(image):
    ...
    edges = [
        int(np.ceil(width * 0.5 - l)),
        int(np.ceil(height * 0.5 - l)),
        int(np.floor(width * 0.5 + l)),
        int(np.floor(height * 0.5 + l)),
    ]
...
def linear_regression(x, y):
    ...
    b = mean_x - a * mean_y
    print(
        "yr = a*y + b  which minimizes  || a*y + b - x ||**2 ,"
        + " x = oracle image  ,  y = image to analyze"
    )
    print("a = ", a, "   b = ", b)
...
def local_sum(matrix, shape):
    p1 = shape[0]
    p2 = shape[1]
    p3 = matrix.shape[0] + 2 * p1
    p4 = matrix.shape[1] + 2 * p2
...
def template_matching(img, oracle):
    ...
    aux = local_sum_img1 - local_sum_img2 * local_sum_img2 / np.float32(
        shape2[0] * shape2[1]
    )
    std_img = np.sqrt(np.clip(aux, 0, np.max(aux)))
    std = np.sqrt(
        np.sum(np.abs(oracle - oracle.mean()) ** 2) / (shape2[0] * shape2[1] - 1)
    )
